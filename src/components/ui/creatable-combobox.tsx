...
import { Combobox as ComboboxPrimitive, ComboboxRootChangeEventDetails } from "@base-ui/react";
...
const isCreatableItem = (item: unknown): item is CreatableItem => {
  return typeof item === "object" && item !== null && (item as CreatableItem).creatable === true;
};
// ─── Internal helpers ─────────────────────────────────────────────────────────
...
function CreatableCombobox({
  ...
  const augmentedItems = (() => {
    ...
    const exactMatch = items.some((item) => toLabel(item).toLocaleLowerCase() === lowered);
    ...
    return createOptionPosition === "first" ? [createItem, ...items] : [...items, createItem];
  })();

  const handleValueChange = (next: unknown, details: ComboboxRootChangeEventDetails) => {
    // multiple select
