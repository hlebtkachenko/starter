...
function trimColorString(color: string, maxLength: number = 20): string {
...
function ColorPicker({ color, onChange }: { color: string; onChange: (color: string) => void }) {
  const [hsl, setHsl] = React.useState<[number, number, number]>([0, 0, 0]);
  ...
  const handleColorChange = (newColor: string) => {
    ...
    if (normalizedColor.startsWith("#")) {
      parsed = hexToHsl(normalizedColor);
    } else {
      const m = normalizedColor.match(/\d+(\.\d+)?/g)?.map(Number) ?? [0, 0, 0];
      parsed = [m[0] ?? 0, m[1] ?? 0, m[2] ?? 0];
    }

    setHsl(parsed);
    onChange(`hsl(${parsed[0].toFixed(1)}, ${parsed[1].toFixed(1)}%, ${parsed[2].toFixed(1)}%)`);
  };
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-[240px] p-3">
        ...
        >
          ...
          >
            <motion.div
              className="absolute size-4 rounded-full border-2 border-background shadow-md"
              style={{
                left: `${hsl[1]}%`,
                top: `${100 - hsl[2]}%`,
          ...
          <div className="flex items-center space-x-2">
            ...
            <Input
              ...
              className="h-8 flex-grow rounded-md border border-input bg-background px-2 text-sm"
