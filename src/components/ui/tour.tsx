...
function Tour(props: TourProps) {
  ...
  const contextValue = React.useMemo<TourContextValue>(() => {
    ...
    if (stepFooter) value.stepFooter = stepFooter;
    if (onPointerDownOutside) value.onPointerDownOutside = onPointerDownOutside;
    if (onInteractOutside) value.onInteractOutside = onInteractOutside;
    if (onOpenAutoFocus) value.onOpenAutoFocus = onOpenAutoFocus;
    if (onCloseAutoFocus) value.onCloseAutoFocus = onCloseAutoFocus;
    return value;
  }, [
    dir,
    alignOffset,
...
function TourStep(props: TourStepProps) {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (onStepEnter) stepData.onStepEnter = onStepEnter;
    if (onStepLeave) stepData.onStepLeave = onStepLeave;
  ...
  const middleware = React.useMemo(() => {
    ...
    return [
      ...
      arrow &&
        onArrow({
          element: arrow,
          ...(stepData.arrowPadding !== undefined ? { padding: stepData.arrowPadding } : {}),
        }),
