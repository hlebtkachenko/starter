...
function KeyValue(props: KeyValueProps) {
  ...
  const contextValue = React.useMemo<KeyValueContextValue>(() => {
    const value: KeyValueContextValue = {
      rootId,
      minItems,
      keyPlaceholder,
      valuePlaceholder,
    ...
    if (onPaste) value.onPaste = onPaste;
    if (onAdd) value.onAdd = onAdd;
    if (onRemove) value.onRemove = onRemove;
    if (onKeyValidate) value.onKeyValidate = onKeyValidate;
    if (onValueValidate) value.onValueValidate = onValueValidate;
    if (maxItems !== undefined) value.maxItems = maxItems;
    return value;
  }, [
    onPaste,
    onAdd,
